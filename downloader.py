...
class Downloader:
    ...
    def str_to_data_uri(self, str):
        return "data:text/plain;charset=utf-8;base64,%s" % base64.b64encode(
            bytes(str, "utf-8")
        ).decode("ascii")
    ...
    def close_driver(self):
        try:
            self.driver.quit()
            self.driver.__class__.__del__ = lambda self: None
        except Exception:
            pass
        finally:
            self.driver = None
    ...
    def download(self):
        ...
        for i in range(total_manga):
            ...
            logging.info("Finished download manga %d, imgdir: %s", i + 1, t_img_dir)
            time.sleep(2)
        self.close_driver()
