...
class CmoaJP(WebsiteActions):
        ...
        return int(
            str(
                driver.execute_script(
                    "return document.getElementById('menu_slidercaption').innerHTML"
                )
            ).split("/")[1]
        )

    def move_to_page(self, driver, page):
        driver.execute_script('SpeedBinb.getInstance("content").moveTo(%d)' % page)
    ...
    def get_imgdata(self, driver, now_page):
        image_elements = driver.find_element(
            By.ID, "content-p%d" % now_page
        ).find_elements(By.CSS_SELECTOR, "img")

        overlaps = self._get_overlaps(driver, now_page)
        imgs_arr = []
        imgs_height = [0]
        for idx, i in enumerate(image_elements):
            ...
            overlap = overlaps[idx] if idx < len(overlaps) else 0
            imgs_height.append(height + imgs_height[-1] - overlap)
    ...
    def _get_overlaps(self, driver, now_page):
        overlaps = driver.execute_script(
            """
            var stripsEl = document.getElementById('content-p' + arguments[0])
                                    .querySelectorAll('img');
            var strips = Array.from(stripsEl);
            var overlaps = [];
            for (var i = 0; i < strips.length - 1; i++) {
                var rect = strips[i].getBoundingClientRect();
                var nextTop = strips[i + 1].getBoundingClientRect().top;
                var scale = strips[i].naturalHeight / strips[i].height;
                var overlap = Math.round((rect.bottom - nextTop) * scale);
